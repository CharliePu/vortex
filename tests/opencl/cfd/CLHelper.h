...
void _clMemcpyH2D(cl_mem dst, const void *src, int size){
	...
	H2D += t2 - t1;
#endif
// #ifdef TIMING  // disabled due to missing probe_event_time implementation
...
void _clMemcpyD2D(cl_mem dst, cl_mem src, int size){
    ...
    cl_event event;
	printf("size=%d\n", size);
	printf("DEBUG: _clMemcpyD2D before clEnqueueCopyBuffer\n"); fflush(stdout);
	oclHandles.cl_status = clEnqueueCopyBuffer(oclHandles.queue, src, dst, 0, 0, size, 0, NULL, &event);
	printf("DEBUG: _clMemcpyD2D after clEnqueueCopyBuffer, status=%d\n", oclHandles.cl_status); fflush(stdout);
#ifdef ERRMSG
	if(oclHandles.cl_status != CL_SUCCESS){
